...
Coin Portfolio Tracker is a full stack iOS + Spring Boot project for advanced cryptocurrency portfolio tracking and analysis. It allows users to perform comprehensive tracking of their cryptocurrency portfolio as well as useful analysis such as future value estimation. It also features a powerful recommendation system (based on coins already in the user's portfolio) and provides a real-time collection of trending news articles (retrieved from Google News). The backend server leverages features such as in-memory caching and rate limiting to improve service efficiency and reliability. 

The frontend app is compatible with devices running iOS 17+.
...
This project is built using Java, Python, Spring Boot, and Swift/SwiftUI. In addition to these primary languages, the project also leverages a variety of supplementary frameworks such as GSON, Bucket4J (rate limiting), Swift Combine, and Swift Charts. 

## Systems

The backend server is deployed on an AWS Linux-based EC2 instance.
